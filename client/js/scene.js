...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.keyboard.on(
      "keydown-E",
      () => !this.dead && sendAction("CONSUME"),
    );
    this.input.keyboard.on(
      "keydown-Q",
      () => !this.dead && sendAction("USE_FUEL"),
    );
  }
  ...
  sendHeldMoves() {
    if (this.dead) return;
    const k = this.keys;
    // Collect the whole cycle and hand it over in one call: the server folds a
    // tick's messages into one capped step, so prediction batches to match.
    const dirs = [];
    if (k.W.isDown) dirs.push(DIR.UP);
    if (k.S.isDown) dirs.push(DIR.DOWN);
    if (k.A.isDown) dirs.push(DIR.LEFT);
    if (k.D.isDown) dirs.push(DIR.RIGHT);
    if (dirs.length) this.sendMove(dirs);
  }
  ...
  renderPlayersAndCamps(players) {
    ...
    for (const p of players) {
      ...
      if (!camp) {
        camp = this.add
          .circle(cpos.px, cpos.py, TILE_SIZE * 0.35, COLOR_FIRE_LIT)
          .setDepth(1);
      ...
      if (!m) {
        ...
        if (isSelf) {
          ...
          const h1 = this.add
            .circle(pos.px, pos.py, TILE_SIZE * 0.14, COLOR_SELF)
            .setDepth(4);
          const h2 = this.add
            .circle(pos.px, pos.py, TILE_SIZE * 0.14, COLOR_SELF)
            .setDepth(4);
    ...
    for (const [sid, m] of this.playerMarkers) {
      ...
      if (c) {
        c.destroy();
        this.campMarkers.delete(sid);
      }
      const hs = this.handMarkers.get(sid);
      if (hs) {
        hs.forEach((h) => h.destroy());
        this.handMarkers.delete(sid);
      }
    }
  }
